...
int max_val = 511; //maximum value - varies between forward and reverse (given differing polaraties)
...
void loop() {
    ...
    if (abs(myGamepad->axisRX()) >= deadspace){
        ...
        map(abs(myGamepad->axisRX()), deadspace, max_val, min_motor_speed, acceleration);
    }

    if (abs(myGamepad->axisY()) >= deadspace){
        leftRight = (myGamepad->axisY() > 0);
        map(abs(myGamepad->axisY()), deadspace, max_val, min_motor_speed, acceleration);
    }
